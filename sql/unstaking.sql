SELECT CONCAT(extract(day from to_timestamp(receipt_included_in_block_timestamp / 10^9)), '/', extract(month from to_timestamp(receipt_included_in_block_timestamp / 10^9)), '/', extract(year from to_timestamp(receipt_included_in_block_timestamp / 10^9))), 
	COUNT(*) as total_unstake_ops, 
	SUM(CAST (args->'args_json'->>'amount' AS numeric)) / 10^24 as total_unstake_vol, 
	COUNT(DISTINCT(receipt_predecessor_account_id)) as total_unstakers, 
